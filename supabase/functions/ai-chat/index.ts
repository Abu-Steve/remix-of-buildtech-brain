...
serve(async (req) => {
  ...
  try {
    ...
    if (accessibleDocuments.length > 0) {
      ...
      for (const doc of accessibleDocuments) {
        ...
        documentSources.push({
          ...
          title: doc.title,
          type: doc.type,
          excerpt: documentContent ? documentContent.substring(0, 200) : (doc.description?.substring(0, 100) || 'Dokument aus der Builtech Brain Wissensdatenbank')
        });
      }
    } else {
    ...
    const systemPrompt = `Du bist der Builtech Brain KI-Assistent für Dokumenten- und Wissensmanagement im Bauwesen.
